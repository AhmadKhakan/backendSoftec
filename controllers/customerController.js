...
const verifyEmailAddress = async (req, res) => {
  console.log("verifyEmailAddress", req.body.email);
  ...
  if (isAdded) {
    ...
    try {
      ...
      const isAdded = await User.findOne({ email });
        ...
        jwt.verify(
          ...
          async (err, decoded) => {
            ...
            if (existingUser) {
              ...
              const hashedPassword = bcrypt.hashSync(password, 10); // Salt rounds set to 10
...
const verifyPhoneNumber = async (req, res) => {
  ...
  try {
    ...
    const isAdded = await User.findOne({ phone: phoneNumber });
